...
|----------|----------|-------------|
| `SEALMETRICS_API_TOKEN` | Yes | Your API authentication token |
| `SEALMETRICS_ACCOUNT_ID` | No | Default account ID (can be specified per query) |

**Alternative authentication:** Instead of `SEALMETRICS_API_TOKEN`, you can use `SEALMETRICS_EMAIL` and `SEALMETRICS_PASSWORD`, though the token method is preferred.
